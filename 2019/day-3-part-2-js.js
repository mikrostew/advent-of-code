...
class Point {
  constructor(x, y, dist_from_start) {
    this.x = x;
    this.y = y;
    // distance from the start of this path
...
class Segment {
  ...
  points(start_point) {
    ...
    switch (this.direction) {
      case 'U':
        // increment y
        return point_range.map(i => new Point(start_point.x, start_point.y + i, start_point.dist_from_start + i));
        break;
      case 'L':
        // decrement x
        return point_range.map(i => new Point(start_point.x - i, start_point.y, start_point.dist_from_start + i));
        break;
      case 'D':
        // decrement y
        return point_range.map(i => new Point(start_point.x, start_point.y - i, start_point.dist_from_start + i));
...
// start at origin
let current_point = new Point(0, 0, 0);
...
path1.forEach(segment_str => {
  ...
  // TODO: this may always use the point with the furthest distance though? may have to change this...
...
path2.forEach(segment_str => {
  ...
  points.forEach(p => {
    if (path1_points[p] !== undefined) {
      ...
      let dist_on_both_paths = path1_points[p].dist_from_start + p.dist_from_start;

      // found an intersection - is this the first one, or the shortest one?
      if (min_distance == 0 || dist_on_both_paths < min_distance) {
        min_distance = dist_on_both_paths;
      }
    }
  });
